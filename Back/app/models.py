...
# ============================================
# 0. СПРАВОЧНИКИ (СТАТИЧЕСКИЕ ТИПЫ)
# ============================================
...
class Users(Base):
    ...
    review_responses = relationship("ReviewResponse", back_populates="user")
    comments = relationship("Comment", back_populates="user")

    # Complaints
    complaints_filed = relationship("Complaint", back_populates="complainant",
                                    foreign_keys="Complaint.complainant_user_id")
    ...
    moderated_violations = relationship("BlocksAndViolations", back_populates="blocked_by_admin_user",
...
# ============================================
# 4. ИЗБРАННОЕ, ОТЗЫВЫ, КОММЕНТАРИИ
# ============================================
...
class Review(Base):
    ...
    author = relationship("Users", back_populates="reviews_given", foreign_keys=[author_id])
    user = relationship("Users", back_populates="reviews_received", foreign_keys=[user_id])
...
class ReviewResponse(Base):
    ...
    user = relationship("Users", back_populates="review_responses")
...
class Chat(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.user_id"), nullable=False)
    agent_id = Column(Integer, ForeignKey("agent_profile.agent_id"), nullable=False)
    title = Column(String(200))
    created_at = Column(DateTime, default=datetime.now)
...
class BlocksAndViolations(Base):
    ...
    blocked_by_admin_user = relationship("Users", back_populates="moderated_violations",
                                         foreign_keys=[blocked_by_admin])
