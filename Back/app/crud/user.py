...
class CRUDUser(CRUDBase[Users]):
    ...
    def get_by_role(self, db: Session, role_id: int, skip: int = 0, limit: int = 100) -> List[Users]:
        return db.query(Users).filter(Users.role_id == role_id).order_by(Users.registration_date.desc()).offset(
            skip).limit(limit).all()

    def update_last_activity(self, db: Session, user_id: int) -> None:
        db.query(Users).filter(Users.user_id == user_id).update(
            {Users.last_activity: func.now()},
