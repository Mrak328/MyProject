...
from app.models import Users, Listing, Address, City
...
def _enrich_listing(listing, db: Session) -> dict:
    """Собрать ответ объявления с фото и адресом"""
...
async def search_listings(
    ...
    sort_map = {
        ...
        "views_desc": Listing.views.desc(),
        "area_asc": Listing.total_area.asc(),
        "area_desc": Listing.total_area.desc(),
        "date_desc": Listing.publication_date.desc(),
        "date_asc": Listing.publication_date.asc(),
    }
    ...
    return {
        "items": [_enrich_listing(l, db) for l in listings],
        "total": total,
        "page": page,
        "size": page_size,
...
async def get_listing_photos(listing_id: int, db: Session = Depends(get_db)):
    ...
    return [
        {"photo_id": p.photo_id, "file_url": p.file_url, "title": p.title}
        for p in photos
    ]
