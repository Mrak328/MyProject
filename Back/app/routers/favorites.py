...
async def remove_favorite(
    ...
    if not deleted:
        raise HTTPException(status_code=404, detail="Объявление не в избранном")
...
async def get_favorites(
    ...
    for fav in favorites:
        ...
        if listing:
            listings.append(_enrich_favorite(listing, db))
    return listings
...
async def check_favorite(
    ...
    return {
        ...
        "is_favorite": favorite_crud.is_favorite(db, current_user.user_id, listing_id)
    }